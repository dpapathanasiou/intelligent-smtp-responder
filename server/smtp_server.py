...
The custom smtp server: respond to all incoming email ...
...
def as_timed_client (cargo, fn):
    ...
    if client_is_idle(cargo[1]['start']):
        cargo[0].wfile.write(bad_request+cr_lf)
        return ('done', cargo)
    else:
        return fn()
...
def greeting (cargo):
    ...
    if not valid_ip_address(ip):
        ...
        stream.wfile.write('220 '+domain_logo+' SMTP'+cr_lf)
        email_data = cargo[1]
        email_data['ip'] = ip
...
def helo (cargo):
    def _helo ():
        ...
        if helo_pattern.search(client_msg) or ehlo_pattern.search(client_msg):
            stream.wfile.write('250 Hello'+cr_lf)
            return ('mail', cargo)
        else:
            stream.wfile.write(bad_request+cr_lf)
            return ('done', cargo)
    ...
    def _mail ():
        ...
        if mail_pattern.search(client_msg):
            ...
            if sender is None or not valid_email_address(sender):
                ...
                stream.wfile.write('250 Ok'+cr_lf)
                email_data = cargo[1]
...
def data (cargo):
    ...
    stream.wfile.write('354 End data with \\r\\n.\\r\\n'+cr_lf)
    ...
    if client_error or len(contents) == 0:
        ...
        stream.wfile.write('250 Ok: queued'+cr_lf)
